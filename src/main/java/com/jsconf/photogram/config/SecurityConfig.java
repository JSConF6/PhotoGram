...
import com.jsconf.photogram.config.oauth.OAuth2DetailsService;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()
                .antMatchers("/", "/user/**", "/image/**", "/subscribe/**", "/comment/**", "/api/**").authenticated() // 해당 URL은 인증이 필요하다.
                .anyRequest().permitAll() // 그 외 다른 요청은 모두 인증 없이 가능
                .and()
                .formLogin()
                .loginPage("/auth/signin") // 인증이 필요한 페이지 요청 시 /auth/signin 으로 이동시킨다. GET
